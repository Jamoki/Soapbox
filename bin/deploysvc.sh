...
if [[ -z "$SVCNAME" || -z "$SSHCONFIG" || -z "$APPCONFIG" ]]; then 
    ...
    echo "BUILDCONFIG is Debug or Release (default)"
    exit 1
fi
...
ssh $SSHCONFIG "cd ${SVCLIBDIR}; echo '<?xml version=\"1.0\" encoding=\"utf-8\"?><configuration><appSettings file=\"app.${APPCONFIG}.config\"></appSettings></configuration>' > ${SVCNAME}Service.exe.config"
if [[ $? -ne 0 ]]; then exit 1; fi
